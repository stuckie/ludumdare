...
AlienGruntClass = {};
AlienGruntClass.__index = AlienGruntClass;

function AlienGruntClass.create( type )
	local unit = {};
	setmetatable(unit, AlienGruntClass);

	unit.type = type;			-- Human or AI (hah)
	unit.class = CLASS_GRUNT;
	...
	unit.weapon2 = WEAPON_ACID_SPLASH;
...
function AlienGruntClass:moveLeft()
	if self.x > 0 then
		local newX = self.x - 1;
		local newY = self.y;
		local newZ = self.z;
...
function AlienGruntClass:moveRight()
...
function AlienGruntClass:moveUp()
	if self.y > 0 then
		local newX = self.x;
		local newY = self.y - 1;
		local newZ = self.z;
...
function AlienGruntClass:moveDown()
...
function AlienGruntClass:floorDown()
...
function AlienGruntClass:floorUp()
...
function AlienGruntClass:fire(x, y, z)
	if self.ap > 1 then
...
function AlienGruntClass:door(x, y, z)
...
function AlienGruntClass:skill1(x, y, z)
	-- aimed shot
end

function AlienGruntClass:skill2(x, y, z)
	-- sniper shot
end

function AlienGruntClass:damage(amount)
	print ("Damage: " .. amount);
...
function AlienGruntClass:setPosition(x, y, z)
	derelictEntities[self.z][self.y][self.x] = nil;	
