...
HumanEngineerClass = {};
HumanEngineerClass.__index = HumanEngineerClass;

function HumanEngineerClass.create( type, player )
	local unit = {};
	setmetatable(unit, HumanEngineerClass);

	unit.type = type;			-- Human or AI (hah)
	unit.class = CLASS_ENGINEER;
	unit.player = player;		-- player I belong to for when changes happen to me
	unit.dead = false;
...
function HumanEngineerClass:moveLeft()
	if self.x > 0 then
		local newX = self.x - 1;
		local newY = self.y;
		local newZ = self.z;
...
function HumanEngineerClass:moveRight()
...
function HumanEngineerClass:moveUp()
	if self.y > 0 then
		local newX = self.x;
		local newY = self.y - 1;
		local newZ = self.z;
...
function HumanEngineerClass:moveDown()
...
function HumanEngineerClass:floorDown()
...
function HumanEngineerClass:floorUp()
...
function HumanEngineerClass:fire(x, y, z)
	if self.ap > 1 then
...
function HumanEngineerClass:door(x, y, z)
...
function HumanEngineerClass:skill1(x, y, z)
	-- hack door
end

function HumanEngineerClass:skill2(x, y, z)
	-- build turret
end

function HumanEngineerClass:damage(amount)
	print ("Damage: " .. amount);
...
function HumanEngineerClass:setPosition(x, y, z)
	derelictEntities[self.z][self.y][self.x] = nil;	
