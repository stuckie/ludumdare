...
AlienQueenClass = {};
AlienQueenClass.__index = AlienQueenClass;

function AlienQueenClass.create( type )
	local unit = {};
	setmetatable(unit, AlienQueenClass);

	unit.type = type;			-- Human or AI (hah)
	unit.class = CLASS_ALIEN_QUEEN;
	unit.dead = false;
...
function AlienQueenClass:moveLeft()
	if self.x > 0 then
		local newX = self.x - 1;
		local newY = self.y;
		local newZ = self.z;
...
function AlienQueenClass:moveRight()
...
function AlienQueenClass:moveUp()
	if self.y > 0 then
		local newX = self.x;
		local newY = self.y - 1;
		local newZ = self.z;
...
function AlienQueenClass:moveDown()
...
function AlienQueenClass:floorDown()
...
function AlienQueenClass:floorUp()
...
function AlienQueenClass:fire(x, y, z)
	if self.ap > 1 then
...
function AlienQueenClass:door(x, y, z)
...
function AlienQueenClass:skill1(x, y, z)
	-- lay egg
end

function AlienQueenClass:skill2(x, y, z)
	-- devour
end

function AlienQueenClass:damage(amount)
	print ("Damage: " .. amount);
...
function AlienQueenClass:setPosition(x, y, z)
	derelictEntities[self.z][self.y][self.x] = nil;	
